...
csvdat = csvread(csvfilename);
t_bias = csvdat(2,1);
t = csvdat(2:end,1) - t_bias;
fs = 1/mean(diff(t));

% get the differential measurement (Vled), remove the bias, then digitize it
ir_raw = csvdat(2:end,4) - bias;
ir_dig = ir_raw > lim;
...
% track the indices of low-freq edges
...
% pulse is triggered. When a sufficient gap is detected (track_limit), we know
...
% export the newly filtered signal
% csvwrite('filtered_signal.csv',ir_filtered'); % (REMOVED. this is terribly slow in Octave)
toc;
% -------------------------------------------------------------------------
...
pulse_idx = [];
counter = 1;
for i = 1:min(length(on_idx), length(off_idx))
